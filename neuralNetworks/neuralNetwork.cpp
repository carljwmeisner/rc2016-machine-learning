...
void Neural_Network_t::buildNetworkPart(Data training, int number_runs, double stepSize){
    ...
    for(int j = 0; j < number_runs; j++){
       ...
       // int acc = getError(inputs,answers);
        //if((j+1)%100 == 0){
            errorAtLastStep = error;
            acc = getError(inputs,answers);
            std::cout << "Error at step " << j+1 << ": " << error << std::endl;
            std::cout << "Accuracy at step " << j+1 << ": " << acc << std::endl;
...
void Neural_Network_t::setWeights(std::vector<arma::mat> weights){
    if(weights.size() != layers.size()-1){
        std::cout << "Error, did not send correct weight size.\n";
    }
    else{
        for(int i = 1; i < layers.size(); i++){
            layers[i]->weights = weights[i-1];
        }
    }
}
